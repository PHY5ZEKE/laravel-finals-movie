    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-neutral-800 border-2 border-neutral-700 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex space-x-4">
                    <a href="{{ route('management.showtime.edit', $showtime->showtime_id) }}"
                        class="px-5 py-2 bg-neutral-600 text-neutral-300 rounded-lg font-semibold hover:bg-neutral-700 transition duration-200">
                        Edit
                    </a>
                    <form action="{{ route('management.showtime.destroy', $showtime->showtime_id) }}" method="POST"
                        onsubmit="return confirm('Are you sure you want to delete this showtime?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="px-5 py-2 bg-red-600 text-neutral-300 rounded-lg font-semibold hover:bg-red-700 transition duration-200">
                            Delete
                        </button>
                    </form>
                </div>
